...
\end{theorem}
\begin{proof}
    \induction{$n=1$}{In this case $O(1) \cong C_2$, and so is generated by a reflection about 0.}
    {$n = k - 1$}{
...
\begin{figure}[hb]
    ...
    \begin{tikzpicture}[scale=2]
        \draw[->] (0, 0) -- (0, 1) node[anchor=east] (en) {$\vec{e_k}$};
        \draw[->] (0, 0) -- (0.7, 0.7) node[anchor=west] (Aen) {$A\vec{e_k}$};
        \draw[dotted] (-0.1, -0.27) -- (0.5, 1.3);
        \draw[->] (Aen.west) -- (en.east) node[anchor=south] {$\vec{v}$};
    \end{tikzpicture}
