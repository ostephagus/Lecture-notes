...
\begin{definition}{Discrete-time Markov Chain}
    ...
    \end{equation*}
\end{definition}
Intuitively, we say that the next move depends only on the current position.
\begin{definition}{Time-homogeneous Markov Chain}
    A Markov Chain is \underline{time-homogeneous} if $\P(X_{n+1} = y | X_n = x)$ is always independent of $n$.
\end{definition}
...
\end{theorem}
\begin{proof}
    First, by the law of total probability we get:
    \begin{align*}
        \P(X_{m+n} &= x_{m+n}, \cdots, X_m = x_m) \\
        &= \sum_{x_0, \cdots, x_{m-1}} \P(X_0 = x_0, \cdots, X_{m+n} = x_{m+n}) \\
        &= \sum_{x_0, \cdots, x_{m-1}} \lambda_{x_0}\Pmat{0}{1} \cdots \Pmat{m+n-1}{m+n} \\
        & \begin{aligned}=\Pmat{m}{m+1} &\cdots \Pmat{m+n-1}{m+n}\\&\times\sum_{x_0, \cdots, x_{m-1}} \lambda_{x_0}\Pmat{0}{1} \cdots \Pmat{m-1}{m}\end{aligned} \\
        &= \Pmat{m}{m+1} \cdots \Pmat{m+n-1}{m+n} \P(X_m = x_m)
    \end{align*}
    using \thmref{thmMarkovPathEquivalence}. Conditioning now on $X_m = i$, we find that $\P(X_m = x_m) = \delta_{ix_m}$ and so we have the required form for $\chain{X} \sim \operatorname{Markov}(\vec{\delta_i}, P)$.

    Now we need to prove independence. That is, for any path and for any indices $m \leq i_1 \leq \cdots \leq i_k$ we show:
    \begin{align*}
        \P(X_{i_1} &= x_{m+1}, \cdots, X_{i_k} = x_{m + k}, X_m = x_m, \cdots, X_0 = x_0 | X_m = i) \\
        &\begin{aligned}=\P(X_{i_1} &= x_{m+1}, \cdots, X_{i_k} = x_{m + k} | X_m = i)\\
            &\times \P(X_m = x_m, \cdots, X_0 = x_0 | X_m = i)\end{aligned}
    \end{align*}
    Note that we only need to consider $x_m = i$ because otherwise both sides are zero.
    \begin{align*}
        \P(X_{i_1} &= x_{m+1}, \cdots, X_{i_k} = x_{m + k}, X_m = x_m, \cdots, X_0 = x_0 | X_m = i) \\
        &= \frac{\P(X_{i_1} = x_{m+1}, \cdots, X_{i_k} = x_{m + k}, X_m = x_m, \cdots, X_0 = x_0)}{\P(X_m = i)} \\
        &\begin{aligned}&= \frac{\lambda_{x_0} \Pmat{0}{1} \cdots \Pmat{m-1}{m}}{\P(X_m = i)}\\
            &\qquad\times\P(X_{i_1} = x_{m+1}, \cdots, X_{i_k} = x_{m+k} | X_m = i)\end{aligned} \\
        &\begin{aligned}=\P(X_{i_1} &= x_{m+1}, \cdots, X_{i_k} = x_{m + k} | X_m = i)\\
            &\times \P(X_m = x_m, \cdots, X_0 = x_0 | X_m = i)\end{aligned}
    \end{align*}
    as required.
...
\begin{align*}
    P(X_n = x_n) &= \sum_{x_0, \cdots, x_{n - 1}} \P(X_0 = x_0, \cdots, X_n = x_n) \\
...
\begin{theorem}
    ...
    \begin{enumerate}
        \item $\P(X_n = x) = \left(\vec{\lambda} P^n\right)_x$ for all $x \in I$.
        \item $\P(X_{m + n} = y | X_m = n) = \left(P^n\right)_{x, y}$ for all $x, y \in I$.
    \end{enumerate}
...
\begin{example}
    ...
    \begin{equation*}
        \begin{pmatrix}1/5 \\ 2/5 \\ 2/5\end{pmatrix}
    \end{equation*}
