...
\begin{document}
\begin{remark}
    From now on, unless otherwise stated, we will assume that any state $\psi$ is normalised, $\inn{\psi}{\psi} = 1$.
\end{remark}
...
\end{equation*}
We need to impose the continuity condition $\chi(\pm a) = 0$. This gives that $A\sin(ka) = 0$ and $B\cos(ka) = 0$. There are two options:
\begin{itemize}
...
\begin{figure}
    ...
    \begin{tikzpicture}[>=Latex, thick]
        ...
        \definecolor{potentialBlue}{RGB}{0, 85, 150}
        \definecolor{reflectedRed}{RGB}{220, 20, 30}
        \definecolor{transmittedGreen}{RGB}{0, 140, 60}
        \definecolor{axisBlack}{RGB}{0, 0, 0}

        % --- Axes ---
        % X-axis
        \draw[->, axisBlack, line width=1.5pt] (-5.5, 0) -- (7, 0) node[right] {\Large $x$};
        % Y-axis
        \draw[->, axisBlack, line width=1.5pt] (0, -1.2) -- (0, 4.5);

        % --- Potential Barrier U(x) ---
        % Drawn in Blue
        \draw[potentialBlue, line width=2pt] (0,0) -- (0,3.5) -- (2.5,3.5) node[midway, above] {\Large $U(x)$} -- (2.5,0);
        
        % Label U_0 on y-axis
        \draw[axisBlack, line width=1.5pt] (-0.15, 3.5) -- (0.15, 3.5);
        \node[left] at (-0.1, 3.5) {\Large $U_0$};

        % Label 'a' on x-axis
        \draw[axisBlack, line width=1.5pt] (2.5, 0.15) -- (2.5, -0.15);
        \node[below, font=\bfseries\Large] at (2.5, -0.1) {$a$};

        % --- Incident Wavepacket (Black) ---
        % Tall, narrow Gaussian on the far left
        \draw[black, line width=1.5pt, samples=100, domain=-5:-2] 
            plot (\x, {2.5 * exp(-(\x + 4)^2 / 0.2)});
        
        % Label
        \node[align=center, anchor=south, black] at (-4, 2.8) {\Large Incident \\ \Large wavepacket};
        \draw[->, black, line width=1.5pt] (-4.5, 2.8) -- (-3.5, 2.8);

        % --- Reflected Wavepacket (Red) ---
        % Shorter, wider Gaussian, moving left
        \draw[reflectedRed, line width=1.5pt, samples=100, domain=-3:0] 
            plot (\x, {1.5 * exp(-(\x + 1.5)^2 / 0.5)});
        
        % Label
        \node[align=center, anchor=south, reflectedRed] at (-1.5, 2.0) {\Large Reflected \\ \Large wavepacket};
        \draw[->, reflectedRed, line width=1.5pt] (-1.0, 2.0) -- (-2.0, 2.0);

        % --- Transmitted Wavepacket (Green) ---
        % Shorter, wider Gaussian, moving right
        \draw[transmittedGreen, line width=1.5pt, samples=100, domain=2.5:6.5] 
            plot (\x, {1.5 * exp(-(\x - 4.5)^2 / 0.6)});
        
        % Label
        \node[align=center, anchor=south, transmittedGreen] at (4.5, 2.2) {\Large Transmitted \\ \Large wavepacket};
        \draw[->, transmittedGreen, line width=1.5pt] (4.0, 2.2) -- (5.0, 2.2);

    \end{tikzpicture}
    \caption{Diagram of Gaussian wavepackets interacting with a potential barrier}
    \label{figPotentialBarrier}
\end{figure}
...
Now consider region 2 where $\{x > 0\}$. Here $U(x) = U_0$.
\begin{equation*}
...
\begin{align*}
    ...
    \eta &= \sqrt{\frac{2m(U_0 - E)}{\hbar^2}}
\end{align*}
and find the general solution:
\begin{equation*}
    \left\{\begin{aligned}
        &e^{ikx} + Ae^{-ikx} && x \leq 0 && \text{(region 1)} \\
        &Be^{-\eta x} + Ce^{\eta x} && x \in (0, a) && \text{(region 2)} \\
        &De^{ikx} + Fe^{-ikx} && x \geq a && \text{(region 3)}
    \end{aligned}\right.
\end{equation*}
For region 1, we have decided to use the constant $1$ in front of the incident wave because this allows us to read off the reflection coefficient directly.
In region 3, we set $F = 0$ due to our initial condition that all particles are initially at $-\infty$.

Note that both regions 1 and 3 give oscillatory solutions, whereas in region 2 we see exponential decay of the intensity of the particle beam.
We can determine the coefficients $A$, $B$, $C$ and $D$ using the continuity of $\chi$ and its first derivative at the points $x \in \{0, a\}$. Of note is the value of the constant $D$:
\begin{equation*}
    D = -\frac{4i\eta k}{(\eta - ik)^2 \exp\left[(\eta + i k)a\right] - (\eta + ik)^2 \exp\left[-(\eta - ik)a\right]}
\end{equation*}
Then the transmission coefficient $T$ is equal to $|D|^2$ (because we set the amplitude of the incident wave to 1):
\begin{equation*}
    T = \frac{4k^2 \eta^2}{(k^2 + \eta^2) \sinh(\eta a) + 4k^2 \eta^2}
\end{equation*}
Then this is non-zero! This is remarkable because we have found that, from a beam of particles, some will manage to get through the potential barrier despite having less energy (which is classically impossible).

Even if $U_0 >> E$, which means $\eta a >> 1$,
\begin{equation*}
    T \to \frac{16k^2\eta^2}{(\eta^2 + k^2)^2} e^{-2\eta a} = \frac{16k^2\eta^2}{(\eta^2 + k^2)^2} e^{-\frac{2a}{\hbar}\sqrt{2m(U_0 - E)}}
\end{equation*}
\subsection{Summary of Unbound States}
In summary, we have given two ways to solve the problem of unbound states, for which the solutions of the TISE are non-normalisable.
\begin{enumerate}
    \item We can consider a \underline{Gaussian Wavepacket}, a linear combination of solutions which is normalisable:
        \begin{equation*}
            \psi_{GP}(x, t) = \int_{-\infty}^{\infty} A_{GP}(k) \psi_k(x, t) dk 
        \end{equation*}
        where $A_{GP}$ is chosen such that $\psi_{GP}$ is normalisable.
        Then, for a scattering potential, we define the reflection and transmission coefficients:
        \begin{align*}
            R &= \lim_{t \to \infty} \int_{-\infty}^{0} |\psi_{GP}(x, t)|^2 dx \\
            T &= \lim_{t \to \infty} \int_{0}^{\infty} |\psi_{GP}(x, t)|^2 dx \\
        \end{align*}
    \item We ignore the problem of normalisability and interpret $\psi_k$ as a beam of particles. We solve the TDSE with $\psi_k$, and compute the flux:
        \begin{equation*}
            J(x, t) = -\frac{i\hbar}{2m} \left(\psi^* \frac{\partial \psi}{\partial x} - \psi \frac{\partial \psi^*}{\partial x}\right)
        \end{equation*}
        Then the transmission and reflection coefficients are given in terms of this flux:
        \begin{align*}
