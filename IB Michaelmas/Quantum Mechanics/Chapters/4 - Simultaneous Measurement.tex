...
\begin{proof}
    ...
    \begin{proofdirection}{$\Leftarrow$}{Suppose that the commutator is zero}
        Let $\psi_i$ be an eigenfunction of $\hat{A}$ with eigenvalue $a_i$.
        \begin{align*}
        ...
        \begin{equation*}
            \hat{A}(\hat{B}\psi_i) = a_i \hat{B} \psi_i
        \end{equation*}
        Since this is true for any eigenfunction $\psi_i$, we conclude that $\hat{B}$ maps any eigenspace $E_i$ of eigenfunctions of $\hat{A}$ with eigenvalue $a_i$ onto itself. 
        Therefore, the restricted map $B|_{E_i}$ is a Hermitian operator of $E_i$. Similarly, because the $E_i$ are complete and thus span $\hilb$, we can find a basis of joint eigenfunctions of $\hat{A}$ and $\hat{B}$.
    \end{proofdirection}
\end{proof}
...
\begin{lemma}[Cauchy-Schwarz inequality]
    ...
    \end{equation*}
    with equality only if $\phi = a\psi$ for $a \in \C$.
...
\end{corollary}
\begin{proof}
    Observe that this fact drops out of the statement of \thmref{thmUncertaintyPrinciple}
\end{proof}
...
\end{corollary}
\begin{proof}
    We found earlier that $\cmmt{x}{p} = i \hbar \hat{I}$, so applying this to \thmref{thmUncertaintyPrinciple} gives the result.
\end{proof}
...
\end{proof}
\begin{examples}{
        Considering different operators $\hat{A}$ in \thmref{thmEhrenfest} gives us results that we can understand from Classical Mechanics:
    }
    \item Taking $\hat{A} = \hat{H}$ gives $\frac{dE}{dt} = 0$, conservation of energy.
    \item Taking $\hat{A} = \hat{P}$ gives:
        \begin{equation*}
            \frac{d}{dt} \E{\hat{p}}_\psi = -\E{\frac{\partial U}{\partial x}}_\psi
        \end{equation*}
    ...
    \item Taking $\hat{A} = \hat{x}$ gives:
        ...
        \end{equation*}
        then this finally gives the intuition for why we call $\hat{p}$ the momentum operator.
\end{examples}
