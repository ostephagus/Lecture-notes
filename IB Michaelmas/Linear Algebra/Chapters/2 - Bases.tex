...
\begin{remarks}
    \item For $S \subseteq V$, $\spn{S} \leq V$. Further, if $W \leq V$ and $S \subseteq W$ then $\spn{S} \leq W$.
...
\end{theorem}
The theorem is saying that we can find a subset $T' \subseteq T$ such that when all other elements of $T$ are replaced with elements of $S$, the resulting set still spans $V$.
\begin{proof}
    We will prove this by an iterative procedure, replacing elements of $T$ with elements of $S$ one at a time, and at each stage preserving spanning.
    Let $S = \{u_1, u_2, \cdots, u_m\}$ and $T = \{v_1, v_2, \cdots, v_n\}$.

    First, if $S$ is empty then we are done. Assume, therefore, that $S$ has an element $u_1$.
    Write $u_1$ as a linear combination of vectors in $T$:
    \begin{equation*}
    ...
    Now at step $k < m$, assume that the set $U_k = \{u_1, \cdots, u_k, v_{k+1}, \cdots, v_n\}$ still spans $V$. Then there must exist $\lambda_i, \mu_j \in \F$ such that:
    \begin{equation*}
        u_{k+1} = \sum_{j=1}^{k} \mu_j u_j + \sum_{i=k+1}^{n} \lambda_i v_i
    \end{equation*}
    We need to show that after re-labelling $\lambda_{k+1} \neq 0$. If we instead had all $\lambda_i$ non-zero, then we would have that $u_{k+1}$ can be found as a linear combination of elements of $S$, which contradicts the linear independence of $S$.

    Now we have that $\lambda_{k+1} \neq 0$, and so:
    \begin{equation*}
        v_{k+1} = \frac{1}{\lambda_{k+1}} \left(u_{k+1} - \sum_{j=1}^{k}\mu_i u_i - \sum_{j=k+2}^{n}\lambda_i v_i\right)
    \end{equation*}
    and $U_{k+1} = \{u_1, \cdots, u_{k+1}, v_{k+2}, \cdots, v_n\}$ spans $V$.

    Therefore, we can proceed by induction until we run out of elements of $S$ or $T$. If we were to run out of elements of $T$, then we would have constructed a subset of $S$ that spans $V$, so the remaining elements of $S$ can be written as linear combinations of elements of $S$ which contradicts linear independence. The process must therefore terminate when running out of elements of $S$, so $|S| \leq |T|$. Now $U_m$ is the required set with:
    \begin{equation*}
        T' = \{v_{m+1}, \cdots, v_n\}
    \end{equation*}
\end{proof}
\begin{remark}
    The choices of which elements of $T$ to remove are not arbitrary because we are re-labelling the elements at each stage.
\end{remark}
\begin{corollary}
...
\begin{definition}{(External) Direct sum}
    ...
    \end{itemize}
\end{definition}
We can chain this operation to define the external direct sum for a finite number of vector spaces. For vector spaces $V_1, \cdots, V_m$, the external direct sum of all of them is denoted:
\begin{equation*}
    \bigoplus_{i = 1}^m V_i
\end{equation*}
