...
\end{remark}
\begin{proof}
    By proposition~\ref{propIsometryInvIsAdj},
    \begin{align*}
...
\begin{proof}
    ...
    Since $A$ is Hermitian, we use lemma~\ref{corHermitianEVals} to get that $\lambda$ is real. Therefore, there must exist an eigenvector $v$ with real eigenvalue $\lambda$. Let $v = a + i b$,
    \begin{equation*}
...
\begin{proof}
    ...
    \induction{$n = 1$}{
        ...
        Let $v_1$ be an eigenvector of $\alpha$, which must exist by lemma~\ref{lemSelfAdjEVecExists}. After possibly rescaling, let $v_1$ have length 1.

        Then $\alpha(\spn{v_1}) \leq v_1$, so we invoke lemma~\ref{lemAdjPresevesSubspaces} to get:
        \begin{equation*}
        ...
        Note that by proposition~\ref{propSelfAdjEValsReal}, all the eigenvectors are real.
    }
\end{proof}
\begin{corollary}
...
\end{remark}
\begin{proof}
    We apply theorem~\ref{thmSpectral} to the vector space $\R^n$ with the standard dot product. $A$ is the self-adjoint linear map.
