...
\begin{proof}
    ...
    \begin{align*}
        ...
        &= \left[\adj(A) A\right]_{j, j}
    \end{align*}
    and so the diagonal entries of $\adj(A) A$ are all $\det(A)$.

    Then for $j \neq k$, replace the $j$th column of $A$ by the $k$th column to obtain $A'$.
    \begin{align*}
        0 &= \det(A')\\
        &= \sum_{i} \adj(A)_{ji} a_{ik} \text{ by expanding along } j\text{th column} \\
        &= \left[\adj(A) A\right]_{jk}
    \end{align*}
    and so the off-diagonal entries are zero.
