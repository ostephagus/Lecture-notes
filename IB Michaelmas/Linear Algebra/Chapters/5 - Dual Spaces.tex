...
\begin{theorem}[Double dual isomorphism]
    Let $V$ be a vector space over $\F$. Then there exists a linear map $E : V \mapsto V^{**}$ (the \underline{natural isomorphism}) given by $E(v)(\theta) = \theta(v)$.
...
\end{proof}
\begin{remarks}
    \item It is common to say that $V = V^{**}$, where the equality is understood to be up to isomorphism with $E$.
