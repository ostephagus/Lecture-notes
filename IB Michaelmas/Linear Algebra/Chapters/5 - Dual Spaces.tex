...
\end{definition}
\begin{examples}{}
    \item Consider $\theta : \R^3 \mapsto \R$ given by $\theta(\vec{x}) = x - 2y + 3z$. Then this is in $(\R^3)^*$.
...
Suppose $V$ is a vector space over a field $\F$ with a basis $B$. For $b \in B$, define $b^* \in V^*$ such that:
\begin{equation*}
    b^* \left(\finitesum_{c \in B} \lambda_i c\right) = \lambda_b,~b^*(c) = \delta_{b, c}
\end{equation*}
...
\end{propositions}
\begin{proof}
    First suppose $\sum_{b \in B}' \lambda_b b^* = \zv$ in $V^*$. Then each basis vector $c \in B$,
    \begin{equation*}
        0 = \left(\sum_{b \in B} \lambda_b b^*\right)(c) = \sum_{b \in B}' \lambda_i b^*(c) = \lambda_c
    \end{equation*}
    and therefore we must have $\lambda_c = 0$. Thus, all the coefficients must be zero.

    For the second claim, we can give a short proof and a constructive proof.

    By theorem~\ref{thmMatrixOfLinMap}, for $V, W$ finite-dimensional,
    \begin{equation*}
...
\begin{example}[Dual bases are more complicated in infinite dimensions]
    Consider the space of polynomial functions $\poly(\R)$. Then we can show that the dual space is isomorphic to $\R^\N$, but we showed on the first example sheet that these are not isomorphic, in contrast to the finite-dimensional case.
\end{example}
\begin{corollary}
...
\end{corollary}
\begin{proof}
    By proposition~\ref{propsDualBasis}, $\dim(V) = \dim(V^*)$. Note $V$ is finite dimensional so we can form a basis $B = \{v_1, \cdots, v_n\}$. We can similarly form the corresponding dual basis $B^* = \{v_1^*, \cdots, v_n^*\}$. Then we extend the isomorphism from the basis to the dual basis by proposition~\ref{propLinMapOfBasis}:
    \begin{align*}
        \Phi : V &\mapsto V^*\\
        \sum_{i=1}^{n} \lambda_i v_i &\mapsto \sum_{i = 1}^n \lambda_i v_i^*
    \end{align*}
\end{proof}
...
\begin{align*}
    ...
    &= \sum_{j=1}^n \lambda_j \mu_j \\
    &= \left([\theta]_{B^*}\right)^T [v]_B
\end{align*}
This finite-dimensional isomorphism is important to understand. We can understand the dual basis as being very similar to the space of \textit{row vectors}. Note that this is not quite true, and actually the dual space is the set of linear maps corresponding to multiplying by a row vector.

If we take an element of $V^*$ and express it as a coordinate vector using the basis vectors in $B^*$, then we have constructed a column vector from the row vector, which is why we need to transpose it (or take a dot product, which is not discussed until later in the course).
\begin{example}[Dual of the space of polynomials]
    Consider the space of polynomials $\poly(\R)$. This has basis:
    \begin{equation*}
        B = \{p_0 = 1, p_1 = x, p_2 = x^2, \cdots\}
    \end{equation*}
    Then we must find a functional $\theta_n(p_m) = \delta_{nm}$. What functional can pick out a unique polynomial?

    We can, rather uninterestingly, define $\theta_n(p_m)$ as above, but we can also use differentiation to define elements in the dual space:
    \begin{equation*}
        \theta_n = \left.\frac{1}{m!}\frac{d^{m}}{dx^{m}}\right|_{x = 0}
    \end{equation*}
\end{example}
...
\begin{definition}{Annihilator}
    ...
    \end{equation*}
\end{definition}
We can understand the annihilator of a given subset $S$ as the set of elements of the dual (row vectors or linear functionals) that send all elements of $S$ to zero.
\begin{propositions}{
...
\begin{proof}
    \begin{enumerate}
        ...
        \item Note that $S \subseteq \spn{S}$ and by the second part, we have immediately that $\spn{S}^0 \leq S^0$.
            ...
            Then suppose $\theta \in S^0, v \in \spn{S}$. Write $v = \finitesum_{s \in S} \lambda_s s$, so:
            \begin{equation*}
                \theta(v) = \finitesum_{s \in S} \lambda_s \theta(s) = 0
            \end{equation*}
...
\begin{definition}{Dual map}
    ...
    \end{align*}
\end{definition}
We can explain this better with the help of a diagram.
\begin{figure}
    \centering
    ...
    \label{figDLMDiagram}
\end{figure}
Figure~\ref{figDLMDiagram} shows the relationship between $\alpha$ and its application to a dual vector. If $\theta$ is a dual vector in $W^*$, we can \textit{cut the corner} of going to $W$ by directly applying $\alpha^*(\theta)$ to an element of $V$ to get an element of the underlying field. This explains the definition $\alpha^*(\theta) = \theta \circ \alpha$.
\begin{propositions}{
    ...
    \item $(\alpha \circ \gamma)^* = \gamma^* \circ \alpha^*$. \label{propDLMSwapsCompose}
...
We want to consider a nice isomorphism that is independent of any basis chosen for $V$. This has to produce a unique element of $V^{**} = \L(V^*, \F)$ for each $v \in V$. Therefore set $v^{**}$ to be the linear map such that $v^{**}(\theta) = \theta(v)$. We note that $v^{**}$ \textit{does} depend on the chosen value of $v$, but it is a function that takes as its argument elements of $V^*$.
...
\begin{proof}
    \begin{subproof}{$E$ is linear}
        ...
        \end{align*}
        Then this is true for all $\theta$, so $E(v + \lambda w) = E(v) + \lambda E(w)$.
    \end{subproof}
    Now suppose $V$ is finite-dimensional.
    \begin{subproof}{$E$ is injective.}
        Suppose $v$ is a non-zero vector in $\ker(E)$. Therefore, for all $\theta \in V^*$, $E(v)(\theta) = 0$. Consider a basis $B$ of $V$ with first element $v$. Let $B^*$ be the usual dual basis of $B$. Now set $\theta = v^*$. Then $0 = \theta(v) = v^*(v) = 1$.
...
\end{proof}
\begin{remarks}
    \item We are able to show injectivity in the infinite-dimensional case if we assume the axiom of choice. However, surjectivity does not hold in general.
    \item This isomorphism is called a \underline{natural isomorphism} because it does not depend on the choice of basis. Compare this isomorphism with the isomorphism $V \mapsto V^*$ which does depend on the basis.
\end{remarks}
Note: Suppose $V$ and $W$ are finite-dimensional vector spaces, let $B$ and $C$ be bases for them. Let $\alpha \in \L(V, W)$. Then:
\begin{align*}
    \alpha^*(\theta)(V) &= \left([\alpha^*(\theta)]_{B^*}\right) [v]_B \\
    &= \left(A^T [\theta]_{C^*}\right)^T [v]_B \\
    &= \left([\theta_{C^*}]\right)^T (A[v]_B) \\
    &=\left([\theta]_{C^*}\right)^T [\alpha(v)]_C
\end{align*}
\begin{proposition}
    Suppose $V$ is a finite-dimensional vector space. Every basis $C$ for $V^*$ is the dual basis to some basis of $V$.
    \label{propDualBasisBackwards}
\end{proposition}
\begin{proof}
    Let $C = \{\theta_1, \cdots, \theta_n\}$ where $n = \dim(V)$.

    Let $C^* = \{\theta_1^*, \cdots, \theta_n^*\} \subseteq V^{**}$.

    Consider the isomorphism $E : V \mapsto V^{**}$ as defined in theorem~\ref{thmNatIsom}. Set $v_i = E^{-1}(\theta_i^*) \in V$. Note now that $B = \{v_1, \cdots, v_n\}$ is a basis of $V$ because $E$ is an isomorphism. For all $i, j$:
    \begin{equation*}
        \theta_i(v_j) = E(v_j)(\theta_i) = \theta_j^*(\theta_i) = \delta_{ij}
    \end{equation*}
    Therefore $B$ is the required basis.
\end{proof}
\begin{proposition}
    Consider $V$ a finite-dimensional vector space and $U \leq V$.

    Under the natural isomorphism, $E(U) = (U^0)^0 \leq V^{**}$.
    \label{thmNatIsomSubset}
\end{proposition}
\begin{proof}
    For $u \in U, \theta \in U^0$,
    \begin{equation*}
        E(u)(\theta) = \theta(u) = 0
    \end{equation*}
    This is true for all such $\theta$, so $E(U) \leq (U^0)^0$.

    Next consider dimensions:
    \begin{align*}
        \dim((U^0)^0) &= \dim(V^*) - \dim(U^0) \\
        &= \dim(V) - \left(\dim(V) - \dim(U)\right) \\
        &= \dim(U) \\
        &= \dim(E(U)) \\
    \end{align*}
    Therefore, since $E(U)$ is a subspace of $(U^0)^0$ and they have the same dimension, they must be equal.
\end{proof}
\begin{remarks}
    \item It is common to say that $V = V^{**}$, because a natural isomorphism is understood to be sufficient for equality.
    \item Under this identification, for $U \leq V$, $U = (U^0)^0$ and for $X \subset V^*$, its annihilator is identified with:
        \begin{equation*}
            X_0 = \subsetselect{v \in V}{\theta(v) = 0~\forall \theta \in X} = \bigcap_{\theta \in X}\ker(\theta)
        \end{equation*}
        Then $\dim(X_0) = n - \dim(\spn{X})$ and every $U \leq V$ is $X_0$ for some $X$, namely $X = U^0$.
\end{remarks}
\end{document}