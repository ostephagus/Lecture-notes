...
\begin{definition}{Vector space}
    A \underline{vector space} over a field $\F$ is an Abelian group $(V, +, \zv)$ equipped with a function:
    \begin{align*}
        \F \times V &\mapsto V \\
...
\begin{propositions}{
    ...
    \item $\alpha$ is injective if and only if $\ker{\alpha} = \{\zv_V\}$ \label{propInjectiveIffKer}
\end{propositions}
\begin{proof}
    \begin{enumerate}
...
\begin{proof}
    \begin{subproof}{$\bar{\alpha}$ is well-defined and injective}
        ...
        \begin{align*}
            ...
            &\iff \alpha(u - v) = \zv_W \\
            &\iff \alpha(u) = \alpha(v) \\
            &\iff \bar\alpha (u + \kera) = \bar\alpha(v + \kera)
        \end{align*}
