...
\end{proof}
\begin{proposition}
    Suppose that $X$ is a random variable, and let $\Sigma$ be countable. Let $g$ be a function $\R \mapsto \R$.
    \begin{equation*}
...
\begin{proposition}
    ...
    \end{equation*}
\end{proposition}
\begin{proof} %TODO: Go over this
    \begin{align*}
        X(\omega) &= \sum_{k=1}^\infty 1(X(\omega) \geq k) \\
        &= \sum_{k=0}^\infty 1(X(\Omega) \geq k)
    \end{align*}
...
\begin{itemize}
    \item If $\Var{(X)} = 0$ the $X$ takes a single value
...
\begin{equation}
    \Cov{\left(\sum_{i \in \N} X_i\right)} = \sum_{i=1}^N \Var{(X_i)} + \sum_{i \neq j}^N \Cov{(X_i, X_j)}
\end{equation}
\begin{lemma}
...
\begin{proof}
    ...
    \end{equation*}
    Now assume wlog $X$ and $Y$ are non-negative (to avoid having to use absolute values). Let $t \in \R$:
    \begin{align*}
...
\begin{definition}{Convex function}
    ...
    \end{equation*}
    That is, $f$ is below any chord that joins $x$ and $y$
\end{definition}
\begin{proposition}
