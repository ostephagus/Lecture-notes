...
\begin{proof}
    ...
    Now assume that $f'(x) = 0$. Then choose $x \in (a, b]$, and apply theorem~\ref{thmMeanValue}, so we have $c \in (a, x)$ sich that $f(x) - f(a) = 0$. That is, $f(x) = f(a)$. And since $x$ was arbitrary, $f(x) = f(a)$ for all $x$.
\end{proof}
\begin{warning}
...
\begin{proof}
    ...
    \begin{subproof}{$f^{-1}$ is differentiable on $(c, d)$ with the given derivative.}
        Let $y \in (c, d)$ adn set $x = f^{-1}(y)$. Given $h$ such that $y + h \in (c, d)$, define $k$ such that $y + h = f(x + k)$
        \begin{align*}
