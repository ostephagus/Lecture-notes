...
\begin{theorem}[Minimality Criterion]
    ...
    \begin{equation*}
        \frac{f_{\vec{X}}(\vec{x}, \theta)}{f_{\vec{X}}(\vec{y}, \theta)} \text{ is constant} \iff T(\vec{x}) = T(\vec{y})
    \end{equation*}
    Then $T$ is a minimal sufficient statistic.
    \label{thmMinCriterion}
\end{theorem}
\begin{proof}
    We need to show that such a statistic is sufficient and minimal. Let $z_t$ be a representative element of $\subsetselect{\vec{x}}{T(\vec{x}) = t}$. We then write:
    \begin{equation*}
        f_{\vec{X}}(\vec{x}, \theta) = f_X(z_{T(\vec{x})}, \theta) \frac{f_{\vec{X}}(\vec{x}, \theta)}{f_{\vec{X}}(z_{T(\vec{x})}, \theta)}
    \end{equation*}
    [\textbf{To be completed next lecture.}]
\end{proof}
\begin{example}
