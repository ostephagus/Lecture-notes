...
\begin{proof}
    ...
    \begin{align*}
        \Var(\tilde{\theta}) &= \E[\Var(\tilde{\theta} | T)] + \Var(\E[\tilde{\theta} | T]) \\
...
\begin{example}
    ...
    Using example~\ref{expUnifMaxEstimator}, we know that $\E\left[\frac{n+1}{n} \max_i X_i\right]$ is unbiased. Therefore,
    \begin{equation*}
...
\begin{definition}{Confidence interval}
    A $\gamma$ or $100\gamma\%$ \underline{confidence interval} for a parameter $\theta$ is a random interval $(A(\vec{X}), B(\vec{X}))$ such that:
    \begin{equation*}
...
\end{definition}
\begin{warning}
    The following is an incorrect interpretation: having observed $\vec{X} = \vec{x}$, there is a $\gamma$ probability that $\theta$ is in $(A(\vec{x}), B(\vec{x}))$.
\end{warning}
\begin{example}
...
\end{remark}
\begin{example}
    Consider $X_i \sim N(0, \sigma^2)$. We want a $95\%$ confidence interval for $\sigma^2$.

    Note that $\frac{X_i}{\sigma} \sim N(0, 1)$, and so:
    \begin{equation*}
        \sum_{i=1}^{n} \frac{X_i^2}{\sigma^2} \sim \chi_n^2
    \end{equation*}
    Then $R(\vec{X}, \sigma^2) = \sum_{i=1}^{n} \frac{X_i^2}{\sigma^2}$ is the pivot. Letting $F(x)$ be the cumulative distribution function for the $\chi_n^2$ distribution, let $c_1 = F^{-1}(0.025)$ and $c_2 = F^{-1}(0.975)$. Therefore:
    \begin{align*}
        0.95 &= \P\left(c_1 \leq \sum_{i=1}^{n} \frac{X_i^2}{\sigma^2} \leq c_2\right) \\
        &= \P\left(\frac{\sum_{i=1}^{n} X_i^2}{c_2} \leq \sigma^2 \leq \frac{\sum_{i=1}^{n} X_i^2}{c_1}\right)
    \end{align*}
\end{example}
\begin{example}
    Suppose $X_i \sim \text{Ber}(p)$, and suppose we want to consider a $95\%$ confidence interval. We consider the maximum likelihood estimator,
    \begin{equation*}
        \hat{p} = \frac{1}{n} \sum_{i=1}^{n} X_i
    \end{equation*}
    Then by the Central Limit Theorem, $\hat{p} \sim N(p, \frac{p(1-p)}{n})$ when $n$ is large. Therefore, we construct a standard normal:
    \begin{equation*}
        \frac{\sqrt{n}(\hat{p} - p)}{\sqrt{p(1-p)}} \sim N(0, 1)
    \end{equation*}
    Then this gives an approximate pivot. Let $z = \Phi^{-1}(0.025)$. Therefore,
    \begin{equation*}
        \P\left(-z \leq \frac{\sqrt{n} (\hat{p} - p)}{\sqrt{p(1-p)}} \leq z\right) \approx 0.95
    \end{equation*}
    Since we are making approximations anyway, we simplify the inversion by taking $p \approx \hat{p}$:
    \begin{align*}
        0.95 &\approx \P\left(-z \leq \frac{\sqrt{n} (\hat{p} - p)}{\sqrt{\hat{p}(1-\hat{p})}} \leq z\right) \\
        &= \P\left(\hat{p} - z\sqrt{\frac{\hat{p}(1-\hat{p})}{n}} \leq p \leq \hat{p} + z\sqrt{\frac{\hat{p}(1-\hat{p})}{n}}\right)
    \end{align*}
\end{example}
\section{Bayesian Estimation}
So far we have been using frequentist methods, wherein the parameter $\theta \in \Theta$ is fixed and unknown, and we draw data from a distribution to estimate $\theta$. We will now consider Bayesian methods where $\theta$ is treated as random with \underline{prior distribution} $\pi(\theta)$. Conditional on $\theta$, the data $\vec{X}$ has pdf $f_{\vec{X}}(\vec{x}, \theta)$.

Given an observation $\vec{X} = \vec{x}$, we combine with the prior distribution to form the \underline{posterior distribution} $\pi(\theta | \vec{x})$.

By Bayes' Rule,
\begin{equation*}
    \pi(\theta | x) = \frac{\pi(\theta) f_{\vec{X}}(\vec{x}, \theta)}{f_{\vec{X}}(\vec{x})}
\end{equation*}
where $f_{\vec{X}}(\vec{x})$ is the marginal distribution of $\vec{X}$,
\begin{equation*}
    f_{\vec{X}}(\vec{x}) =
    \begin{cases}
        \int_\Theta f_{\vec{X}} (\vec{x}, \theta) \pi(\theta) d\theta & \text{$\theta$ continuous} \\
        \sum_{\theta \in \Theta} f_{\vec{X}} (\vec{x}, \theta) \pi(\theta) & \text{$\theta$ discrete} \\
    \end{cases}
\end{equation*}
Given that, in Bayesian estimation, we treat $\vec{X} = \vec{x}$ as fixed, we can consider the denominator $f_{\vec{X}}(\vec{x})$ as simply a proportionality constant. Then:
\begin{equation}
    \pi(\theta | \vec{x}) \propto \pi(\theta) f_{\vec{X}}(\vec{x})
    \label{eqnBayesDists}
\end{equation}
\begin{remark}
    By the factorisation criterion, we can consider the posterior distribution as dependent on $\vec{x}$ only through a sufficient statistic. The dependence on $\vec{x}$ is treated simply as a proportionality constant.
\end{remark}
\begin{example}
    Suppose that $\theta \in [0, 1]$ is the mortality rate for a procedure at a hospital. In the first 10 operations, there are no deaths. In other hospitals, the mortality rate is between $0.03$ and $0.2$ with an average of $0.1$.

    Consider the prior distribution:
    \begin{equation*}
        \pi(\theta) \sim \text{Beta}(3, 27)
    \end{equation*}
    then this has mean $0.1$, and $\P(0.03 \leq \theta \leq 0.2) = 0.9$ under the distribution $\pi$.

    Let $X_i \sim \text{Ber}(\theta)$ be the indicator for the $i$th patient. We have that $\vec{x} = \vec0$.
    \begin{equation*}
        f_{\vec{X}}(\vec{x}, \theta) = \theta^{\sum_{i=1}^n X_i} (1-\theta)^{\sum_{i=1}^{n} X_i}
    \end{equation*}
    and so the posterior distribution is given by:
    \begin{align*}
        \pi(\theta | \vec{x}) &\propto \theta^{a-1} (1-\theta)^{b-1} \theta^{\sum_{i=1}^{n} X_i} \theta^{n-\sum_{i=1}^{n} X_i} \\
        &\propto \theta^{\sum_{i=1}^{n} X_i + a - 1} (1 - \theta)^{n - \sum_{i=1}^{n} X_i + b - 1} \\
        &= \theta^{\sum_{i=1}^{n} X_i + 2} (1 - \theta)^{n - \sum_{i=1}^{n} X_i + 26} \\
        &= \theta^{2} (1 - \theta)^{36} \\
        &\sim \text{Beta}(3, 37)
    \end{align*}
    Now our mean is $\frac{3}{40}$, and the overall distribution has been biased further towards 0.

    In this example, the prior and posterior were from the same family of distributions, known as conjugacy. This means we simply update the parameters of the prior distribution to get to the posterior, rather than ending up with a different and complicated distribution.
\end{example}
\subsection{Decision Theory}
Suppose we must choose a decision $\delta \in D$, and consider a loss function $L(\theta, \delta)$ which gives loss incurred by making a decision $\delta$ when the true parameter is $\theta$.

Then the Von-Neumann-Morgensten Theorem, says that, under certain assumptions, the best choice of $\delta$ is the minimiser of the expected loss under the posterior distribution. Let $L(\theta, \delta)$ be the expected loss over the posterior distribution $\pi(\theta | \vec{x})$.

\begin{definition}{Bayes estimator}
    The \underline{Bayes estimator} $\hat{\theta}_{B}$ is defined by:
    \begin{align*}
        \hat{\theta}_B &= \argmin_{\delta \in D} h(\delta) \\
        \text{Where } h(\delta) &= \int_{\Theta} L(\theta, \delta) \pi(\theta | \vec{x}) d\theta
    \end{align*}
\end{definition}
\begin{example}
    Consider the \underline{quadratic loss}:
    \begin{equation*}
        L(\theta, \delta) = (\theta - \delta)^2
    \end{equation*}
    The expected loss is then:
    \begin{equation*}
        h(\delta) = \int_{\Theta} (\theta - \delta)^2 \pi(\theta | \vec{x}) d\theta
    \end{equation*}
    and so differentiating this gives:
    \begin{equation*}
        \int_\Theta (\theta - \delta) \pi(\theta | \vec{x}) d\theta
    \end{equation*}
    and we find:
    \begin{equation*}
        \delta = \int_\Theta \theta \pi(\theta | \vec{x})
    \end{equation*}
    is the Bayes estimator. This is $\E[\theta]$ under $\pi(\theta | \vec{x})$.
\end{example}
\begin{example}
    Consider the \underline{absolute loss}, $L(\theta, \delta) = |\theta - \delta|$.
    \begin{align*}
        h(\delta) &= \int_\Theta |\theta - \delta| \pi(\theta | \vec{x}) d\theta \\
        &= \int_{-\infty}^{\delta} -(\theta - \delta) \pi(\theta | \vec{x}) d\theta + \int_{\delta}^{\infty} (\theta - \delta) \pi(\theta | \vec{x}) d\theta \\
        &= \int_{-\infty}^{\delta} -\theta \pi(\theta | \vec{x}) d\theta + \int_{\delta}^{\infty} \theta \pi(\theta | \vec{x}) d\theta + \delta \int_{-\infty}^{\delta} \pi(\theta | \vec{x}) d\theta - \delta \int_{\delta}^{\infty} \pi(\theta | \vec{x}) d\theta
    \end{align*}
    Then differentiating with respect to $\delta$ gives:
    \begin{equation*}
        h'(\delta) = \int_{-\infty}^{\delta} \pi(\theta | \vec{x}) d\theta - \int_{\delta}^{\infty} \pi(\theta | \vec{x}) d\theta 
    \end{equation*}
    And so this is zero when $\delta$ is the median of the distribution.
\end{example}
\begin{example}
    Consider $X_i \sim N(\mu, 1)$ for $i = 1$ to $n$.

    Let the prior distribution of $\mu$ be $N(0, \tau^{-2})$ for known $\tau > 0$.
    \begin{align*}
        \pi(\mu | \vec{x}) &\propto f_{\vec{X}}(\vec{x}, \mu) \pi(\mu) \\
        &\propto \exp\left(-\frac12 \sum_{i=1}^{n} (x_i - \mu)^2\right) \exp\left(-\frac{\mu^2\tau^2}{2}\right) \\
        &\propto \exp\left(-\frac12 (n + \tau)^2 \left(\mu - \frac{\sum_{i=1}^{n} x_i}{n + \tau^2}\right)^2\right)
    \end{align*}
    and so we conclude that:
    \begin{equation*}
        \mu | \vec{x} \sim N\left(\frac{\sum_{i=1}^{n} x_i}{n + \tau^2}, \frac{1}{n+\tau^2}\right)
    \end{equation*}
    The posterior mean and median are both the same (since this is a normal distribution), so the quadratic and absolute loss functions would give the same estimator.

    We can write this as a convex combination of $\hat{\theta}_{\text{MLE}}$ and zero:
    \begin{equation*}
        \hat{\theta}_B = \frac{n}{n+\tau^2} \underbrace{\frac{\sum_{i=1}^{n}x_i}{n}}_{\hat{\theta}_{\text{MLE}}} + \frac{\tau^2}{n + \tau^2} (0)
    \end{equation*}
    which tells us that the Bayesian estimator is biased towards zero for smaller $n$, which is the contribution from the prior distribution.
\end{example}
\begin{definition}{Credible interval}
    A $\gamma$ or $100\gamma\%$ \underline{credible interval} $(A(\vec{X}), B(\vec{X}))$ satisfies:
    \begin{equation*}
