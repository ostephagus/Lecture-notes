...
Then these field $\vec{E}$ and $\vec{B}$ must satisfy the wave equation. This reduces to the dispersion relation:
\begin{equation*}
    \omega^2 = c^2 k^2
\end{equation*}
...
\begin{align*}
    ...
    &= 2E_0 \sin(kx) \sin(\omega t)
\end{align*}
which indeed solves the required boundary condition. We can interpret this as a standing wave with a node at $x = 0$.

%TODO: Work out $\vec{B}$ and show that it is discontinuous at the boundary. We can therefore work out a surface current density $\vec{K}(t)$. Solution is in the notes.
...
\subsection{Energy in Electrogmagnetic Waves}
Consdier:
\begin{align*}
