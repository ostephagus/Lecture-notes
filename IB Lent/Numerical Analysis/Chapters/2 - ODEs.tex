...
\begin{remark}
    ...
    \begin{equation*}
        \frac{\rho(w)}{\sigma(w)} = \log(w) + O\left((w-1)^{p-1}\right)
    \end{equation*}
\end{remark}
\begin{proof}
    \begin{align*}
...
\begin{definition}{Root condition}
    A polynomial $\rho$ satisfies the \underline{root condition} if all the roots of $\rho$ lie inside the disc $\subsetselect{z \in \C}{|z| \leq 1}$, and any roots with $|z| = $ are simple.
\end{definition}
...
\end{example}
\begin{example}[Failure of Root Condition I]
...
\end{example}
\begin{example}[Failure of Root Condition II]
    Consider:
    \begin{equation*}
...
\end{equation*}
for an $s$-step method, or:
\begin{equation*}
    \frac{\rho(w)}{\sigma(w)} - \log(w) + O((w-1)^{s + 2})
\end{equation*}
which has order $s+1$.

If we require an explicit method, the order decreases by $1$ to $s$.
...
\begin{example}[Adams Methods]
    ...
    \begin{align*}
        ...
        \sigma(w) &= -\frac12 + \frac23 w + \frac{5}{12} w^2
    \end{align*}
...
\begin{equation*}
    \vec{y}(t_{n+1}) = \vec{y}(t_n) + \int_{t_n}^{t_{n+1}} \vec{y}'(\tau) d\tau = \vec{y}(t_n) + \int_{t_n}^{t_{n+1}} \vec{f}(\tau, \vec{y}(\tau)) d\tau 
\end{equation*}
