...
\subsection{Truncation Error and Order}
...
\end{examples}
\subsection{Alternative Ways to Compute Order}
Since the Taylor expansion for polynomials of degree $p$ contains only terms of $O(h^k)$ with $k \leq p$, an equivalent condition to equation~\ref{eqnMultistepOrder} is:
\begin{equation}
    \sum_{l=0}^{s} \rho_l Q(t_{n+l}) = h \sum_{l=0}^{s} \sigma_l Q'(t_{n+l})~~\forall Q \in \P_p[x]
    \label{eqnMultistepOrder2}
\end{equation}
then the coefficients $\rho_l$ and $\sigma_l$ can be determined by choosing appropriate polynomials $Q$ in \eqnref{eqnMultistepOrder2}.
...
\begin{theorem}[Dahlquist Equivalence Theorem]
    ...
    \begin{enumerate}
        \item the order $p$ of the method is at least 1;
