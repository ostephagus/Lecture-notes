...
\begin{equation}
    \int_{a}^{b} w(x) f(x) dx \approx \sum_{k=1}^{\nu} \underbrace{b_k}_{\text{weights}} f(\underbrace{c_k}_{\text{nodes}})
    \label{eqnGaussQuadrat}
\end{equation}
So we want to find a method to obtain the weights $b_k$ and the nodes $c_k$. We also want to find the ``best'' approximation. We will do this by ensuring equality for polynomials up to degree $n$, and trying to find the largest value for $n$.
...
\begin{proof}
    ...
    Therefore $m \geq n$. However, %TODO: Continue. We end up with $m = n$.
\end{proof}
