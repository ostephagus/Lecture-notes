...
\begin{definition}{Simply connected domain}
    A connected domain $D$ is \underline{simply connected} if, for any closed curve in $D$, the interior of the curve is a subset of $D$.
\end{definition}
\begin{remark}
...
\end{figure}
\begin{proof}
    Consider figure~\ref{figContourShrink}. By \thmref{thmCauchy}, we know that:
    \begin{align*}
        0 &= \int_{\gamma_1 + \tilde{\gamma} + (-\gamma_2) + (-\tilde{\gamma})} f(z) dz \\
        &= \int_{\gamma_1} f(z) dz - \int_{\gamma_2} f(z) dz
    \end{align*}
    when we take the gap to be infinitesimal. Therefore,
    \begin{equation*}
        \int_{\gamma_1} f(z) dz = \int_{\gamma_2} f(z) dz
    \end{equation*}
\end{proof}
