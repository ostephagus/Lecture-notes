...
\begin{proposition}
    ...
    where
    \begin{equation*}
        a_m = \frac{1}{2\pi i} \oint_{\gamma} \frac{f(z)}{(z-z_0)^{m+1}} dz
    \end{equation*}
    for any contour $\gamma$ in the annulus. This is convergent within the annulus and uniformly convergent within compact subsets of the annulus.
...
\end{proposition}
\begin{proof}
    Let $z_0 = 0$ after possibly translating the argument. Let $z \in \C$ with $R_1 < r_1 < |z| < r_2 < R_2$. Let $\gamma_1, \gamma_2$ be anticlockwise, circular contours of radii $r_1, r_2$.

    As we have previously seen in figure~\ref{figContourShrink}, we can bridge these two contours with radial lines in the opposite direction, to create $\gamma = \gamma_2 - \gamma_1$ in the limit as the bridge gets infinitely thin.
...
\begin{enumerate}
    ...
    \begin{enumerate}
        ...
        \item if there exists $N > 0$ such that for all $m < -(N+1)$ then $a_m = 0$, and $a_{-N} \neq 0$ (that is, the Laurent series terminates for negative $N$), then $f$ has a pole of order $N$ at $z = z_0$;
...
\end{proposition}
\begin{proof}
    First, $f(z)$ has a Taylor series around $z_0$ that starts at $(z - z_0)^m$:
    \begin{align*}
        f(z) &= \sum_{n = m}^{\infty} \frac{f^{(n)}(z_0)}{n!} (z - z_0)^n \\
        &= (z - z_0)^m \underbrace{\sum_{n=0}^{\infty} \frac{f^{(n+m)}(z_0)}{(n+m)!} (z - z_0)^n}_{h(z)}
    \end{align*}
    Then $h(z)$ is analytic around $z_0$ and since the zero of $f$ has order $m$, the term in $(z - z_0)^m$ must be non-zero, and so $h(z_0) \neq 0$. Therefore, we can consider $\tilde{h}(z) = \frac1{h(z)}$ which is also analytic around $z_0$.
    \begin{align*}
        g(z) &= (z - z_0)^{-m} \tilde{h}(z) \\
        &= (z - z_0)^{-m} \sum_{n=0}^{\infty} \frac{\tilde{h}^{(n)}(z_0)}{n!} (z - z_0)^n \\
        &= \sum_{n=-m}^{\infty} a_n (z - z_0)^n
    \end{align*}
    and we know $a_{-n} \neq 0$ because $\tilde{h}(z_0) \neq 0$.
\end{proof}
\begin{theorem}[Grand Picard Theorem]
...
\begin{proof}
    ...
    \begin{align*}
        ...
        &= \sum_{m = -\infty}^{\infty} a_m \int_0^{2\pi} r^m e^{im\theta} i r e^{i\theta} d\theta \\
        &= \sum_{m = -\infty}^{\infty} a_m \int_0^{2\pi} r^{m+1} ie^{i(m+1)\theta} d\theta \\
    \end{align*}
