...
\begin{example}
    ...
    \begin{equation*}
        ...
        \end{pmatrix}
    \end{equation*}
    then in this case, player 1 thinks they should play $i = 2$, and player 2 thinks they should play $j = 1$, so both converge on the cell $A_{21} = 3$.
\end{example}
\begin{example}
...
Note that, by theorem~\ref{thmLinearProgStrongDual}, the Nash Equilibrium exists for a well-defined problem (feasible and bounded).
...
\begin{theorem}
    ...
    \begin{enumerate}
        ...
        \item The dual is feasible: $A\vec{q} \leq v\vec{e}, \vec{e}^T\vec{q} = 1, \vec{q} \geq 0$,
...
Consider $n$ suppliers and $m$ consumers. Let supplier $i$ produce $s_i$ goods, and consumer $j$ demand $d_j$ goods. Require that these demands are met:
\begin{equation*}
...
\begin{definition}{Tranport problem}
    ...
    \begin{align*}
        ...
        & x_{ij} \geq 0~\forall (i, j) \in \{1, \cdots, n\} \times \{1, \cdots, m\}
    \end{align*}
\end{definition}
...
\begin{proof}
    ...
    \begin{equation*}
        \sum_{k : (j, k) \in E} \overline{M}_{jk} - b_j
    \end{equation*}

    Now suppose we send a flow $x_{ij}$ from producer $(i, j)$ to consumer $j$. Then the flow from this producer to consumer $i$ is the rest: $\overline{M}_{ij} - x_{ij}$. Because this excess is transported at $0$ cost, the overall cost is:
    \begin{equation*}
        \sum_{(i, j) \in E} c_{ij} x_{ij}.
    \end{equation*}
    Now consider the constraints of the new transport problem.
    
    Consumer $i$ now has inflow:
    \begin{equation*}
        \sum_{k:(i, k) \in E} \left(\overline{M}_{ik} - x_{ik}\right) + \sum_{k : (k, i) \in E} x_{ki}
    \end{equation*}
    which must equal the demand, so:
    \begin{equation*}
...
\begin{definition}{Cut Capacity}
    The \underline{capacity} of a graph cut is the total capacity of vertices from $S$ to $V \backslash S$:
    \begin{equation*}
